...
// ============================================================
// FIX: window.open() must happen synchronously inside the click
// handler, or browsers block it as a popup. We open a blank tab
// first (this is still within the user gesture), then navigate
// it to the real URL once the Firestore write has finished.
async function handleStopClick(stopNumber, key) {
  ...
  try {
    ...
    if (newTab) {
      ...
      // Popup blocked entirely even for the blank tab — fall back
      // to opening in the same tab so the click still does something.
